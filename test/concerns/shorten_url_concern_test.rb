...
describe "Validators::ShortenURL" do
  ...
  it "Should respond created if shortened" do
    @context.redis.expects(:call).with("SET", @valid_attributes[:shortcode], @valid_attributes[:url]).returns(true)
  ...
  it "Should respond with error if not shortened" do
    @context.redis.expects(:call).with("SET", @valid_attributes[:shortcode], @valid_attributes[:url]).returns(false)
